...
void atender_memory_stick(int fd_memory_stick) {
    int size;
    int* ptr_memory_stick_size = recibir_mensaje(fd_memory_stick, &size);
    if (ptr_memory_stick_size == NULL) return;

    int memory_stick_size = *ptr_memory_stick_size;
    free(ptr_memory_stick_size);

    char* memory_stick_ip = recibir_mensaje(fd_memory_stick, &size);
    char* memory_stick_puerto = recibir_mensaje(fd_memory_stick, &size);

    // IMPORTANTE: Se registra en el administrador de memoria
    conectar_memory_stick(memory_stick_ip, memory_stick_puerto, (uint32_t)memory_stick_size, fd_memory_stick);

    log_info(logger, "## Memory Stick de %d bytes conectada (Total acumulado: %u bytes)", 
             memory_stick_size, obtener_memoria_total());
