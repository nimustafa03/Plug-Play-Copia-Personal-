// =============================================================
//  cpu_2.c  —  Módulo CPU
//  Cómo ejecutar: ./bin/cpu cpu_2.config [identificador]
//
//  Responsable CP1:
...
/*------------------------- INICIO CPU 2 -------------------------*/
	...
	t_log* logger_cpu_2 = iniciar_logger();
	log_info(logger_cpu_2, "Ejecutando_CPU_2");

    // TODO Adriel: cargar config con config_create()
    log_info(logger_cpu_2, "Creando .config");
    t_config* config_cpu_2 = iniciar_config();

    //Muestra los valores del archivo .config
	char* ip_ks = config_get_string_value(config_cpu_2,"KS_IP");
	char* ip_km = config_get_string_value(config_cpu_2,"KM_IP");
    char* ip_ms = config_get_string_value(config_cpu_2,"MS_IP");
	char* puerto_ks = config_get_string_value(config_cpu_2,"KS_PORT");
	char* puerto_km = config_get_string_value(config_cpu_2,"KM_PORT");
	char* puerto_ms = config_get_string_value(config_cpu_2,"MS_PORT");
	log_info(logger_cpu_2,"IP KS: %s - IP KM: %s - IP MS: %s",ip_ks, ip_km, ip_ms);
	log_info(logger_cpu_2,"PUERTO KS: %s - PUERTO KM: %s - PUERTO MS: %s",puerto_ks,puerto_km,puerto_ms);
 
/*----------------- CONEXION CON KERNEL SCHEDULER -----------------*/

    // int fd_ks = crear_conexion(KS_IP, KS_PORT);
    log_info(logger_cpu_2, "Intentando conexion con Kernel Scheduler...");
	int fd_ks = crear_conexion(ip_ks, puerto_ks);
	log_info(logger_cpu_2,"CPU 2 Conectada con KS");
...
/*------------------ CONECCION CON KERNEL MEMORY ------------------*/
    ...
    log_info(logger_cpu_2, "Intentando conexion con Kernel Memory...");
	fd_ks = crear_conexion(ip_km, puerto_km);
	log_info(logger_cpu_2,"CPU 2 Conectada con KM");
    // enviar handshake MSG_HANDSHAKE_CPU


/*------------------- CONECCION CON MEMORY STICK ------------------*/

    log_info(logger_cpu_2, "Intentando conexion con Memory Stick...");
	fd_ks = crear_conexion(ip_ms, puerto_ms);
	log_info(logger_cpu_2,"CPU 2 Conectada con MS");
